...
from algo_test_utils import get_test_all_csvs, get_ece_black_test_df
import warnings
...
parser = argparse.ArgumentParser(description='Run inference to check res') # singlepadlut05_modelclean = 0.79, singlepadlut05 = 0.67 singlepadlut0_modelclean = 0.81 singlepadlut0 = 0.77
parser.add_argument('--version_folder', default='singlepadv0.12allmodel', type=str) #singlepadv0.6impl   singlepadv0.9 padgroupv1.10rgbwhite
parser.add_argument('--version', default='v0.12final', type=str)

parser.add_argument('--confidence', default='all', type=str)
parser.add_argument('--output_type', default='dual2', type=str)  # before_pos_uncertain, all_black
parser.add_argument('--valdataset', default='calibrate_test', type=str, help='bbtest, bbtestmz, alltestval, newval, debug, cur_jiraissue, led_cross_pair, jiraissues')
...
parser.add_argument('--region', default=f'singlepad', type=str)   #padgroup
...
parser.add_argument('--sharpness_factor', default=5, type=int)
...
parser.add_argument('--if_calcu_ECE', default=False, type=bool)
parser.add_argument('--ECE_calcu_type', default='Multi_ECE', type=str, choices=['ECE', 'Ada_ECE', 'Multi_ECE', 'Multi_Ada_ECE'])
...
model_prediction_dir = os.path.join(f'./{args.region}/{args.valdataset}')
...
compare_version_res_folder = f'./results/{version_folder}_{date}'
...
#             img_color = version_folder.split('_')[3]
    ...
    val_image_pair_data, n_class, insp_label_list, ref_image_name_list, defect_code, defect_decode, defect_full_decode, val_image_pair_res, binary_labels, ref_labels, \
           insp_labels, ref_image_batches, insp_image_batches, insp_image_name_list = get_ece_black_test_df(args, 
                                                                            ...
                                                                            transform,
                                                                            sharpness_save)
    defect_decode_dict = {i:v  for i, v in zip(defect_decode.keys(), defect_code.keys())}
    ...
    for version_name in version_name_list:
        ...
        if args.if_calcu_ECE:
            # 计算ECE
            ECE_metrictor = get_ECE_calculator(args)
            # 计算校准误
            output_bos_th = torch.from_numpy(output_bos_np)
            output_bom_th = torch.from_numpy(output_bom_np)
            bos_labels = torch.from_numpy(binary_labels).type(torch.int64)
            bom_labels = torch.from_numpy(insp_labels)
            bos_ECE_results_save_path = os.path.join(ckp_folder, args.valdataset, 'bos')
            bom_ECE_results_save_path = os.path.join(ckp_folder, args.valdataset, 'bom')

            os.makedirs(bos_ECE_results_save_path, exist_ok=True)
            os.makedirs(bom_ECE_results_save_path, exist_ok=True)

            # if 'Multi' not in args.ECE_calcu_type
            defect_decode_bos_dict = {0: 'ok', 1: 'ng'}

            bos_ece_dict, bos_mce_dict = ECE_metrictor(output_bos_th, bos_labels, save_path = bos_ECE_results_save_path, 
                          vis_acc_conf  = True, vis_bin_ece  = True, defect_decode_dict = defect_decode_bos_dict)
            
            bom_ece_dict, bom_mce_dict = ECE_metrictor(output_bom_th, bom_labels, save_path = bom_ECE_results_save_path, 
                          vis_acc_conf  = True, vis_bin_ece  = True, defect_decode_dict = defect_decode_dict)
        ...
        total_class = np.unique(insp_labels)
        defect_decode_keys = defect_decode.keys()
        
        if -1 in defect_decode_keys:
            pass
        assert len(defect_decode_keys) - 1 == output_bom_np_softmax[0].size, f"标签有误"
        
        model_version_infos = version_name.split('_ckp_best')[-1]
        model_cersion = model_version_infos.split('cp05')[0] + '_' + model_version_infos.split('dual2')[-1].split('.pth')[0]
        csv_save_path = os.path.join(model_prediction_dir, model_cersion + '.csv')
        
        cur_val_image_pair_data = val_image_pair_data.copy()
        cur_val_image_pair_data['pair_key'] = cur_val_image_pair_data['ref_image'].astype(str) + '_' + cur_val_image_pair_data['insp_image'].astype(str)
        records = []
        for index, img_names  in enumerate(zip(ref_image_name_list, insp_image_name_list)):
            ref_image_name, insp_img_name = img_names
            
            pair_key = ref_image_name + '_' + insp_img_name
            ok_conf, ng_conf = output_bos_np_softmax[index]
            mclass_conf = output_bom_np_softmax[index]


            record = {
                'pair_key': pair_key,
                'model_bi_ok_conf': ok_conf,
                'model_bi_ng_conf': ng_conf,
            }
            for defect_idx, defect_label in defect_decode.items():
                if defect_idx == -1:
                    continue
                record[f"multi_class_{defect_label}"] = mclass_conf[defect_idx]
            
            records.append(record)
        records_df = pd.DataFrame(records)
        cur_val_image_pair_data = cur_val_image_pair_data.merge(records_df, on='pair_key', how='left')
        cur_val_image_pair_data.to_csv(csv_save_path)

        #     loc_str = (val_image_pair_data['ref_image'].astype(str).str.contains(ref_image_name) 
        #                              & val_image_pair_data['insp_image'].astype(str).str.contains(insp_img_name))
        #     assert loc_str.sum() == 1, "图像pair对唯一匹配"
        #     val_image_pair_data.loc[loc_str, 'model_bi_ok_conf'] = ok_conf
        #     val_image_pair_data.loc[loc_str, 'model_bi_ng_conf'] = ng_conf

        #     for defect_idx, defect_label in defect_decode.items():
        #         if defect_idx == -1:
        #             continue
        #         # print(defect_label)
        #         val_image_pair_data.loc[loc_str, f"multi_class_" + defect_label] = mclass_conf[defect_idx]

        # val_image_pair_data.to_csv(csv_save_path)
...
#     all_store_df_res.to_csv(os.path.join(ckp_folder, f'{region}_{args.valdataset}_{version_folder}_{rs_img_size_w}{rs_img_size_h}_lutp{lut_p}_{args.light_device}_{img_color}_{args.img_type}.csv'))#f'./csv/{region}_{args.valdataset}_{version_folder}_{rs_img_size_w}{rs_img_size_h}_lutp{lut_p}.csv'))