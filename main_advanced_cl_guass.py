...
import time
import os
from dataloader.image_resampler import mp_weighted_resampler, ImageLoader2, stratified_train_val_split, \
...
#                                                          f'aug_train_pair_labels_{region}_240913_final_white.csv'),
                                            ...
                                            #  os.path.join(img_folder, 'merged_annotation', date,
                                            # f'body_white_paris_final_train_update.csv'),
                                            #  os.path.join(img_folder, 'merged_annotation', date,
                                            # f'body_rgb_paris_final_train_update.csv'),
                                             os.path.join(img_folder, 'merged_annotation', date,
...
#             os.path.join(img_folder, 'merged_annotation', date, f'aug_test_pair_labels_{region}_240819_final_white.csv')
                                            ...
                                            # os.path.join(img_folder, 'merged_annotation', date,
                                            # f'body_white_paris_final_test_update.csv'),
                                            #  os.path.join(img_folder, 'merged_annotation', date,
                                            # f'body_rgb_paris_final_test_update.csv'),
                                            os.path.join(img_folder, 'merged_annotation', date,
    ...
    if run_mode.startswith('train'):
        ...
        train_dataset = ImageLoader2(img_folder, Xtrain_resampled, ytrain_resampled, ybinary_resampled,
                                    ...
                                    transform=transforms.Compose([
                                        ...
                                        transforms.GaussianBlur(kernel_size=args.guass_k, sigma=(0.1, args.guass_maxsigma)),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_traintestval_infos', default=False, type=bool,
                        help='是否保存训练的数据集信息')
    parser.add_argument('--guass_k', default=3, type=float,
                        help='图片压缩概率')   
    parser.add_argument('--guass_maxsigma', default=2.0, type=float,
                        help='高斯模糊gmma最大值')
