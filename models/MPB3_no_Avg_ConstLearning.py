...
class MPB3net(nn.Module):
    ...
    def __init__(self, backbone='resnet18', r=16, pretrained=False, n_class=9, n_units=[128, 128], output_form='dual'):
        ...
        if backbone.startswith('sepost'):
            ...
            if 'resnet' in backbone:
                ...
                self.cnn_encoder = nn.Sequential(feature_extractor.features)
                ...
                if 'CLL' in self.output_form:
                ...
                elif 'CLC' in self.output_form:
                    self.cl_conv1 = nn.Sequential(
                        nn.Conv2d(int(feature_channel), int(feature_channel), kernel_size=3, stride=1, padding=1),
                    ...
                    self.cnn_encoder.append(feature_extractor.avgpool)
                    self.cnn_encoder.append(nn.Flatten())

                if 'Pre' not in self.output_form:
    ...
    def forward(self, x_1, x_2):
        ...
        elif 'Pre'in self.output_form:
            feature_2 = self.cnn_encoder(x_2)
            feature_1 = self.cnn_encoder(x_1)

            feature_2 = self.cl_conv2(feature_2)
            feature_1 = self.cl_conv1(feature_1)

            if self.backbone.startswith('mobilenetv3'):
