...
class MPB3net(nn.Module):
    ...
    def __init__(self, img_c, backbone='resnet18', r=16, pretrained=False, n_class=9, n_units=[128, 128], output_form='dual'):
        ...
        if 'resnet18' in backbone:
            ...
            # 获取原始第一层
            old_conv = feature_extractor.features[0][0]  # 第一个卷积层是 features[0][0]

            # 创建新的 Conv2d 层，in_channels 改为 4，其他参数保持不变
            new_conv = nn.Conv2d(
                in_channels=img_c,
            ...
            feature_extractor.features[0][0] = new_conv
        ...
        if backbone.startswith('sepost'):
            ...
            if 'resnet' in backbone:
                ...
                if 'semerge' in  backbone:
                    from models.utilities import SEMerge
                    self.cnn_encoder = feature_extractor.features
                    self.mergeblock = SEMerge(c = feature_channel, o = feature_channel, r = r)
                
                elif 'cbammerge' in backbone:
                    from models.utilities import CBAMMerge

                    self.cnn_encoder = feature_extractor.features
                    self.mergeblock = CBAMMerge(in_planes = feature_channel, reduction = r)

                else:
                    from models.utilities import CATmerge
                    self.cnn_encoder = nn.Sequential(feature_extractor.features, feature_extractor.avgpool, nn.Flatten())
                    self.mergeblock = CATmerge()
                # branch of similarity
                self.head_bos = nn.Sequential(
                    nn.Linear(int(feature_channel * bos_c_multiplier), bos_fc_out[0]),
