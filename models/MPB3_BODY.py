...
class MPB3net(nn.Module):
        ...
        if backbone.startswith('sepost'):
            ...
            if 'resnet' in backbone:
                ...
                if 'semerge' in  backbone:
                    from models.utilities import SEMerge  # mobienet中自带有SE（conv来实现的，参数少），所以加SE（线形层，参数多）感觉比较鸡肋
                    print(f"merge block is SEMerge")
                    
                    self.cnn_encoder = feature_extractor.features
                    self.mergeblock = SEMerge(c = feature_channel, o = feature_channel, r = r)
                
                elif 'csmerge' in backbone:
                    from models.utilities import CBAMMerge
                    print(f"merge block is CBAMMerge")
                    self.cnn_encoder = feature_extractor.features
    ...
    def forward(self, x_1, x_2):
        ...
        feature_concat =self.mergeblock(feature_1, feature_2)
