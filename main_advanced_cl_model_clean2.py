...
def main(args):
    ...
    if region == 'component':
        ...
        annotation_filename, val_annotation_filename, test_annotation_filename, aug_train_pair_data_filenames, aug_val_pair_data_filenames = \
            get_train_csv(annotation_folder, region, version_name, special_data = args.special_data)
        if 'white' in version_name:
            print(version_name)
            rs_factor = 1.14
            rs_same_offset = 10
    ...
    if run_mode.startswith('train'):
        ...
        if aug_train_pair_data_filenames is not None:
            ...
            Xtrain_resampled_, ytrain_resampled_, ybinary_resampled_, material_train_resampled_ = load_pair_csv(aug_train_pair_data_filenames, 
        ...
        train_dataset = ImageLoader(img_folder, Xtrain_resampled, ytrain_resampled, ybinary_resampled,
                                    ...
                                    transform=transforms.Compose([  #  非同步的，所以金板检测板旋转后角度可能不同，裁剪也可能不同
...
if __name__ == '__main__':
    ...
    parser.add_argument('--defect_dirs', default=['NONE', 'WRONG'], type=list)
