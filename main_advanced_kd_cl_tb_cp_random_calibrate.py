...
def train_ece(train_loader, model, criterion_bos, criterion_bom, cl_loss, optimizer, gpu_exists, ECE_Calcutor=None, use_amp=False, scaler=None, kd_config=None):
    losses = AverageMeter()
...
def train_no_cl_ece(train_loader, model, criterion_bos, criterion_bom, cl_loss, optimizer, gpu_exists, ECE_Calcutor=None, use_amp=False, scaler=None, kd_config=None):
    losses = AverageMeter()
    accuracies = AverageMeter()
...
def val_ece(val_loader, model, criterion_bos, criterion_bom, gpu_exists, visualize=False, use_amp=False, selection_score='recall', ECE_Calcutor=None):
    ...
    if ECE_Calcutor is not None:
        ...
        label2_all = torch.cat(label_mclass_list, dim=0)

        bi_ece_dict, bi_mce_dict = ECE_Calcutor(output_bos_all, label_binary_all)
        bom_ece_dict, bom_mce_dict = ECE_Calcutor(output_bom_all, label2_all)
...
def val_no_cl_ece(val_loader, model, criterion_bos, criterion_bom, gpu_exists, visualize=False, use_amp=False, selection_score='recall', ECE_Calcutor=None):
...
def main(args):
    ...
    if aug_val_pair_data_filenames is not None:
        ...
        Xval_, yval_, ybinary_val_, material_val_, aug_val_pair_data_df = process_csv_files(test_annotation_filenames,
                                                                                         args, 
                                                                                         label_confidence, 
                                                                                         defect_class_considered, 
                                                                                         defect_code_link, 
    ...
    if args.criterion_func == 'Focal_loss':
    ...
    elif args.criterion_func == 'ECE_loss':
        criterion_bos = ECELoss()
        criterion_bom = ECELoss()
    elif args.criterion_func == 'Adaptive_ECE_loss':
        criterion_bos = AdaptiveECELoss()
        criterion_bom = AdaptiveECELoss()
    # 计算校准误差
    if args.ECE_calcu_type == 'ECE':
        ECE_matric = ECEer().calculate_ece_mce
    elif args.ECE_calcu_type == 'Ada_ECE':
        ECE_matric = ECEer().calculate_ada_ece_mce
    elif args.ECE_calcu_type == 'Multi_ECE':
        ECE_matric = ECEer().calculate_multi_ece_mce        
    elif args.ECE_calcu_type == 'Multi_Ada_ECE':
        ECE_matric = ECEer().calculate_multi_ada_ece_mce  
    else:
        print(f'未指定校准误差函数')
        ECE_matric = None
# 
    ...
    if kd_ratio > 0:
        ...
        val_losses, val_acc_bi, val_acc_mc, val_recall_bi, val_recall_mclass, val_label_positions, val_imgs = val_ece(
            val_loader, teacher_model, criterion_bos, criterion_bom, gpu_exists, visualize=True, use_amp=use_amp,
            selection_score=selection_score, ECE_Calcutor = ECE_matric)

        test_loss, test_acc_bi, test_acc_mc, test_recall_bi, test_recall_mclass, test_label_positions = val_ece(
            test_loader,
            teacher_model,
            criterion_bos,
            criterion_bom,
            gpu_exists, use_amp=use_amp, selection_score=selection_score, ECE_Calcutor = ECE_matric)
    ...
    if run_mode.startswith('train'):
        ...
        if args.resume is None:
            ...
            if "CL" in output_type:
                val_losses, val_acc_bi, val_acc_mc, val_recall_bi, val_recall_mclass, val_label_positions, val_imgs = val_ece(
                    val_loader, model, criterion_bos, criterion_bom, gpu_exists, visualize=True, use_amp=use_amp, 
                    selection_score=selection_score, ECE_Calcutor = ECE_matric)
                ...
                val_losses, val_acc_bi, val_acc_mc, val_recall_bi, val_recall_mclass, val_label_positions, val_imgs = val_no_cl_ece(
                    val_loader, model, criterion_bos, criterion_bom, gpu_exists, visualize=True, 
                    use_amp=use_amp, selection_score=selection_score, ECE_Calcutor = ECE_matric)

                test_loss, test_acc_bi, test_acc_mc, test_recall_bi, test_recall_mclass, test_label_positions = val_no_cl_ece(
                    test_loader,
                    model,
                    criterion_bos,
                    criterion_bom,
        ...
        for epoch in range(start_epoch, epochs):
            ...
            if 'CL' in output_type:
                ...
                train_loss, train_acc = train_ece(train_loader, model, criterion_bos, criterion_bom, cl_loss, optimizer, gpu_exists, ECE_matric, 
                                            use_amp, scaler, kd_config)
                epoch_time = time.time() - epoch_time_start
                scheduler.step()
                # validate the model
                # with torch.no_grad():
                val_loss, val_acc_bi, val_acc_mc, val_recall_bi, val_recall_mclass, val_label_positions = val_ece(val_loader,
                                                                                                            model,
                                                                                                            criterion_bos,
                                                                                                            criterion_bom,
                                                                                                            gpu_exists,
                                                                                                            ...
                                                                                                            ECE_Calcutor = ECE_matric)

                test_loss, test_acc_bi, test_acc_mc, test_recall_bi, test_recall_mclass, test_label_positions = val_ece(
                    test_loader, model, criterion_bos, criterion_bom, gpu_exists, use_amp=use_amp, selection_score=selection_score, ECE_Calcutor = ECE_matric)

            else:
                epoch_time_start = time.time()
                train_loss, train_acc = train_no_cl_ece(train_loader, model, criterion_bos, criterion_bom, cl_loss, optimizer, gpu_exists, ECE_matric, 
                                            use_amp, scaler, kd_config)
                epoch_time = time.time() - epoch_time_start
                scheduler.step()
                # validate the model
                # with torch.no_grad():
                val_loss, val_acc_bi, val_acc_mc, val_recall_bi, val_recall_mclass, val_label_positions = val_no_cl_ece(val_loader,
                                                                                                            model,
                                                                                                            criterion_bos,
                                                                                                            criterion_bom,
                                                                                                            gpu_exists,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--special_data', default=False, type=bool, help="选择不同的数据集配置")

    parser.add_argument('--criterion_func', default='Focal_loss', type=str, choices=['Focal_loss', 'ECE_loss', 'Adaptive_ECE_loss'])
    parser.add_argument('--ECE_calcu_type', default='ECE', type=str, choices=['ECE', 'Ada_ECE', 'Multi_ECE', 'Multi_Ada_ECE'])
