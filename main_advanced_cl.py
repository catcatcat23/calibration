...
import time
import os
from dataloader.image_resampler import mp_weighted_resampler, ImageLoader, stratified_train_val_split, \
    generate_val_pairs, DiscreteRotate, generate_test_pairs, LUT
...
from util import parse_bash_param, load_pair_csv
...
def main(args):
    ...
    if region == 'component':
        ...
        test_annotation_filename = os.path.join(img_folder, 'merged_annotation', date, f'bbtest_labels_{region}resorted.csv')
        ...
        aug_val_pair_data_filenames = [
            ...
            os.path.join(img_folder, 'merged_annotation', date,
                                                      f'aug_test_pair_labels_{region}_240930_final_white.csv')
        ...
        # annotation_filename = os.path.join(img_folder, 'merged_annotation', date, f'aug_train_pair_labels_body_nonpaired_resorted_model_cleaned_250123.csv')
        # val_annotation_filename = os.path.join(img_folder, 'merged_annotation', date, f'aug_val_pair_labels_body_nonpaired_resorted_model_cleaned_250123.csv')
        # test_annotation_filename = os.path.join(img_folder, 'merged_annotation', date, f'bbtest_labels_pairs_body_nonpaired_resorted_model_cleaned_250123.csv')
...
#                                                          f'aug_train_pair_labels_{region}_240913_final_white.csv'),
                                             ...
                                             os.path.join(img_folder, 'merged_annotation', date,
                                                      f'aug_train_pair_labels_{region}_241023_final_white_20573.csv'),
                                             os.path.join(img_folder, 'merged_annotation', date,
                                                      f'aug_train_pair_labels_{region}_241023_final_white_20583.csv'),
                                             os.path.join(img_folder, 'merged_annotation', date,
                                                      f'aug_train_pair_labels_{region}_241023_final_white_240071.csv'),

                                             # LED cross pair 241030
                                            os.path.join(img_folder, 'merged_annotation', date,
                                            f'LED_cross_pairs_ok_train.csv'),
                                            os.path.join(img_folder, 'merged_annotation', date,
                                            f'LED_cross_pairs_ng_train.csv'),                                         

                                            #  melf cross pair 241031
                                            os.path.join(img_folder, 'merged_annotation', date,
                                            f'body_241031_melf_ng_cross_pair2_train.csv'),
                                            os.path.join(img_folder, 'merged_annotation', date,
                                            f'body_241031_melf_ok_cross_pair2_train.csv'),
                                            os.path.join(img_folder, 'merged_annotation', date,
                                            f'aug_train_pair_labels_body_241118_final_white.csv'),
                                            ...
                                            os.path.join(img_folder, 'merged_annotation', date,
                                             ...
                                             # 按照不同料号组合的错件，经过测试，下面文件用于训练
                                            os.path.join(img_folder, 'merged_annotation', date,
                                             f'defect_labels_body_white_pairs_aug_ng_test_soft_bad_size2.csv'), 
                                            os.path.join(img_folder, 'merged_annotation', date,
                                                        f'aug_train_pair_labels_body_250407_final_white_DA855_jlc_dropoldpairs.csv'),
                                            os.path.join(img_folder, 'merged_annotation', date,
                                                      f'aug_train_pair_labels_body_250407_final_white_DA855_jlc_dropoldpairs_cross_pair_ok.csv'),
            ...
            aug_val_pair_data_filenames = [
                                    os.path.join(img_folder, 'merged_annotation', date, f'aug_val_pair_labels_body_nonpaired_resorted_model_cleaned_250123.csv'),
                                    os.path.join(img_folder, 'merged_annotation', date, f'aug_train_pair_labels_body_nonpaired_resorted_model_cleaned_250123.csv'),
                                        os.path.join(img_folder, 'merged_annotation', date, f'aug_test_pair_labels_body_250407_final_white_DA855_jlc_dropoldpairs.csv'),
                                        os.path.join(img_folder, 'merged_annotation', date, f'aug_val_pair_labels_{region}_mergedresorted.csv'),
                                           ...
                                           os.path.join(img_folder, 'merged_annotation', date,
                                                      f'aug_test_pair_labels_{region}_241023_final_white_20573.csv'),
                                           os.path.join(img_folder, 'merged_annotation', date,
                                                      f'aug_test_pair_labels_{region}_241023_final_white_20583.csv'),
#             os.path.join(img_folder, 'merged_annotation', date, f'aug_test_pair_labels_{region}_240819_final_white.csv')
                                              # LED cross pair 241030
                                            os.path.join(img_folder, 'merged_annotation', date,
                                            f'LED_cross_pairs_ng_val.csv'),
                                            os.path.join(img_folder, 'merged_annotation', date,
                                            f'LED_cross_pairs_ok_val.csv'),

                                            # melf cross pair 241031
                                            os.path.join(img_folder, 'merged_annotation', date,
                                            f'body_241031_melf_ng_cross_pair2_val.csv'),
                                            os.path.join(img_folder, 'merged_annotation', date,
                                            f'body_241031_melf_ok_cross_pair2_val.csv'),
                                            os.path.join(img_folder, 'merged_annotation', date,
                                            f'aug_test_pair_labels_body_241118_final_white.csv'),
                                            ...
                                            # 按照不同料号组合的错件，经过测试，下面文件用于训练
                                            os.path.join(img_folder, 'merged_annotation', date,
                                             f'defect_labels_body_white_pairs_aug_ng_train_size_match2.csv'),
                                            os.path.join(img_folder, 'merged_annotation', date,
                                             f'defect_labels_body_white_pairs_aug_ng_train_hard_bad_size2.csv'),  
                                           ]
            # bdg = 'bad case generation'
            if 'bdg' in version_name:
                aug_train_pair_data_filenames.append(os.path.join(img_folder, 'merged_annotation', date,
                                                         f'body_white_paris_final_train_refine2.csv'))
                aug_val_pair_data_filenames.append(os.path.join(img_folder, 'merged_annotation', date,
    ...
    if len(args.specila_issue_list) > 0:
        ...
        specila_issues = parse_bash_param(args.specila_issue_list)
    ...
    if run_mode.startswith('train'):
        ...
        if run_mode == 'train_ft':
            ...
            # Xtrain_resampled, ytrain_resampled, ybinary_resampled, material_train_resampled = mp_weighted_resampler(
            #     train_annotation_df, k=k_factor, batch_size=batch_size, n_max_pairs=n_max_pairs)
        defect_dirs = parse_bash_param(args.defect_dirs)
        if aug_train_pair_data_filenames is not None:
            Xtrain_resampled_, ytrain_resampled_, ybinary_resampled_, material_train_resampled_ = load_pair_csv(aug_train_pair_data_filenames, defect_class_considered, defect_code_link, label_confidence, defect_dirs)
    ...
    try:
        ...
        # add in additional paired data for training
        print(f"generate_test_pairs failed")
        
        Xtest, ytest, ybinary_test, material_test = load_pair_csv([test_annotation_filename], defect_class_considered, defect_code_link, label_confidence, defect_dirs)
    ...
    print(f'n_test_pairs = {len(test_annotation_df)}')
    ...
    if aug_val_pair_data_filenames is not None:
        ...
        Xval_, yval_, ybinary_val_, material_val_ = load_pair_csv(aug_val_pair_data_filenames, defect_class_considered, defect_code_link, label_confidence, defect_dirs)
    ...
    if run_mode.startswith('train'):
        ...
        for epoch in range(start_epoch, epochs):
            ...
            if epoch % verbose_frequency == 0:
                print(f'Ep {epoch + 1}: train loss={train_loss:.3f}, train acc={train_acc:.3f}%,'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_train_trace_pdf', default=False, type=bool,
                        help='是否用pdf保存训练记录')
    parser.add_argument('--img_color', default='white', type=str, choices=['rgb', 'white'], help='只对padgroup有用')
