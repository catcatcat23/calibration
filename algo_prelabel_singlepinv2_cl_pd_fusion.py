...
import numpy as np
from utils.utilities import TransformImageFusion, split
...
parser = argparse.ArgumentParser(description='Run inference to check res') #  = 0.79, singlepadlut05 = 0.67 singlepadlut0_modelclean = 0.81 singlepadlut0 = 0.77
parser.add_argument('--version_folder', default='padgroupv1.22.6rgbwhitelut05mergerbg', type=str) #singlepadv0.6impl   singlepadv0.9 padgroupv1.10rgbwhite
parser.add_argument('--version', default='v1.22.6fusion2', type=str)  #v0.12final
parser.add_argument('--confidence', default='certain', type=str)
parser.add_argument('--output_type', default='dual2', type=str)
parser.add_argument('--valdataset', default='3D', type=str, help='bbtest, bbtestmz, alltestval, newval, debug, cur_jiraissue, led_cross_pair, jiraissues')
...
parser.add_argument('--region', default=f'padgroup', type=str)   # padgroup
...
if 'lut05ed' in args.version_folder:
    args.fusion_type = args.version_folder.split('lut05ed')[-1]
else:
    args.fusion_type = args.version_folder.split('lut05')[-1]


if args.fusion_type == 'catgray':
    args.fusion_type = 'cat_gray'
elif args.fusion_type == 'cathwrw':
    args.fusion_type = 'cat_HW_rgb_white'
elif args.fusion_type == 'cathwrw':
    args.fusion_type = 'cat_HW_white_rgb'
elif args.fusion_type == 'cathwwr':
    args.fusion_type = 'cat_HW_white_rgb'
elif args.fusion_type == 'diffrw':
    args.fusion_type = 'diff_rgb_white'
elif args.fusion_type == 'diffwr':
    args.fusion_type = 'diff_white_rgb'
elif args.fusion_type == 'diffabs':
    args.fusion_type = 'diff_rgb_white_abs'
elif args.fusion_type == 'mergerbg':
    args.fusion_type = 'merge_rb_G'
    ...
    for version_name in version_name_list:
        ...
        if inference_model_mode == 'torch':
            ...
            if 'cbam' in backbone_arch and 'CL' not in version_name:
                ...
                model = MPB3net(img_c, backbone=backbone_arch, pretrained=False, n_class=n_class, n_units=n_units, output_form=output_type)
