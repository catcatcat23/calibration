...
class GradCAM():
    ...
    def forward(self, ref_img, insp_img, cam_type = 'bos'):
        self.grads = []
        self.fmaps = []
        ...
        target_fmap = self.fmaps[0][0].clone().detach().cpu().numpy().squeeze()
        target_grad = self.grads[1][0].clone().detach().cpu().numpy().squeeze()
        target_img = insp_img.clone().detach().cpu().numpy().squeeze()
        self.insp_cam_mask = self._compute_cam_mask(target_img, target_fmap, target_grad)
        
        target_fmap = self.fmaps[1][0].clone().detach().cpu().numpy().squeeze()
        target_grad = self.grads[0][0].clone().detach().cpu().numpy().squeeze()
        target_img = ref_img.clone().detach().cpu().numpy().squeeze()
    ...
    def show_contrastive_results(self, ref_img, insp_img, save_path):
...
def parser_model_param(model_path, region):
    ...
    n_class, rs_img_size_w, rs_img_size_h = init_defect_code(region, model_path)

    return backbone_arch, n_units, n_class, rs_img_size_w, rs_img_size_h
...
#             defect_code_slim = {'ok': 0, 'missing': 1, 'wrong': 3}            
    ...
    return n_class, rs_img_size_w, rs_img_size_h
...
def load_data(data_path, csv_path, date, rs_img_size_h, rs_img_size_w, batch_size = 256):
    ...
    return ref_image_batches, insp_image_batches, ref_image_path_list, insp_image_path_list
...
def test_gradCAM(args, model_path, model, gradCAMer, ref_image_batches, insp_image_batches, ref_image_path_list, insp_image_path_list):
    ...
    for ref_img, insp_img, ref_image_path, insp_image_path in zip(ref_image_batches, insp_image_batches, ref_image_path_list, insp_image_path_list):
        ...
        gradCAMer.forward(ref_img, insp_img, args.cam_type)
...
def GradCAMer(args):
    ...
    backbone_arch, n_units, n_class, rs_img_size_w, rs_img_size_h = parser_model_param(model_path, args.region)
    ...
    ref_image_batches, insp_image_batches, ref_image_path_list, insp_image_path_list = load_data(args.data_path, args.csv_path, args.date, rs_img_size_h, rs_img_size_w, batch_size = 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--region', type=str, default='padgroup', help = '缺陷类型')
    parser.add_argument('--model_path', type=str, 
                        default='./models/checkpoints/padgroupv1.13select/padgroupfcdropoutmobilenetv3largers128128s42c2val0.1b256_ckp_bestv1.13.0slimmaskedf1certainrgbwhitelut0520.0j0.4lr0.025nb128nm128dual2top0.pth.tar', 
                        help = '')
    # singlepinpadv2.8/singlepinpadmobilenetv3smallrs12832s42c3val0.15b256_ckp_bestv2.8.0f1certain20.0j0.4lr0.05nb256nm256dual2last.pth.tar

    parser.add_argument('--target_module', type=str, default='cnn_encoder', help = '要进行gradcam的层')
    parser.add_argument('--target_layer', type=str, default='0', help = '要进行gradcam的层')
    parser.add_argument('--cam_type', type=str, default='bos', help = '对二分类还是多分类结果计算gradcam')
    ...
    parser.add_argument('--date', type=str, default='241022', help = 'csv文件所在文件夹名称')
    parser.add_argument('--data_path', type=str, default='/mnt/dataset/xianjianming/data_clean/', help = '数据路径')
    parser.add_argument('--csv_path', type=str, default='aug_test_pair_labels_padgroup_241224_final_rgb_mask_DA575and577_drop.csv', help = 'csv路径')
