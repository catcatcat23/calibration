...
def train_iter(train_loader_list, model, criterion_bos, criterion_bom, cl_loss, optimizer, gpu_exists, use_amp=False, scaler=None, kd_config=None):
    losses = AverageMeter()
...
def main(args):
    ...
    if region == 'singlepinpad':
        ...
        # annotation_filename = os.path.join(annotation_folder, f'bbtest_labels_pairs_{region}_230306v2_rgb_final3.csv')
        # val_annotation_filename = os.path.join(annotation_folder, f'bbtest_labels_pairs_{region}_230306v2_rgb_final3.csv')
        ...
        aug_train_pair_data_filenames_rgb = [
                                        os.path.join(annotation_folder,
        ...
        aug_val_pair_data_filenames_rgb = [
                                        os.path.join(annotation_folder,
        ...
        aug_train_pair_data.append(aug_train_pair_data_filenames_rgb)
        aug_val_pair_data.append(aug_val_pair_data_filenames_rgb)

        if 'masked' in version_name:

            aug_train_pair_data_filenames_masked = [os.path.join(annotation_folder,
                                            ...
                                            os.path.join(annotation_folder,
                                                        f'aug_train_pair_labels_padgroup_250115_final_rgb_mask_DA656_dropoldpairs.csv'), 
                                              ]

            if 'NGonly' in version_name:
                aug_train_pair_data_filenames_masked += [
                    os.path.join(annotation_folder,
            ...
            aug_val_pair_data_filenames_masked = [os.path.join(annotation_folder,
            ...
            aug_train_pair_data.append(aug_train_pair_data_filenames_masked)
            aug_val_pair_data.append(aug_val_pair_data_filenames_masked)

        if 'rgbwhite' in version_name:

            aug_train_pair_data_filenames_rgbwhite = [
                                             os.path.join(annotation_folder,
                                            ...
                                            os.path.join(annotation_folder,
                                                           f'aug_train_pair_labels_padgroup_250115_final_white_DA656_dropoldpairs.csv'),  
                                                           

                                             ]
            aug_val_pair_data_filenames_rgbwhite = [
                                           os.path.join(annotation_folder,
            ...
            aug_train_pair_data.append(aug_train_pair_data_filenames_rgbwhite)
            aug_val_pair_data.append(aug_val_pair_data_filenames_rgbwhite)

            if 'masked' in version_name:

                aug_train_pair_data_filenames_rgbwhite_masked = [os.path.join(annotation_folder,
                                                ...
                                                os.path.join(annotation_folder,
                                                              f'aug_train_pair_labels_padgroup_250115_final_white_mask_DA656_dropoldpairs.csv')

                                                              
                                                  ]

                aug_val_pair_data_filenames_rgbwhite_masked = [os.path.join(annotation_folder,
                ...
                aug_train_pair_data.append(aug_train_pair_data_filenames_rgbwhite_masked)
                aug_val_pair_data.append(aug_val_pair_data_filenames_rgbwhite_masked)
        ...
        # annotation_filename和val_annotation_filename仅用于debug，训练中不用
    ...
    if len(args.specila_issue_list) > 0:
        aug_train_pair_data_filenames_special_issue = []
        aug_val_pair_data_filenames_special_issue = []
        ...
        for jira_issue_name in specila_issues:
            ...
            for special_train_csv in special_train_csvs:
                aug_train_pair_data_filenames_special_issue.append(os.path.join(annotation_folder, special_train_csv))

            for special_val_csv in special_val_csvs:
                aug_val_pair_data_filenames_special_issue.append(os.path.join(annotation_folder, special_val_csv))

        aug_train_pair_data.append(aug_train_pair_data_filenames_special_issue)
        aug_val_pair_data.append(aug_val_pair_data_filenames_special_issue)

    assert len(aug_train_pair_data) == len(aug_val_pair_data), f"训练集和测试集数量不一致"
    ...
    if annotation_filename is not None:
        ...
        if args.data_type == '3d':
            train_dataset_ori = None
            train_loader_ori = None

        else:
            train_dataset_ori = ImageLoader2(img_folder, Xtrain_resampled, ytrain_resampled, ybinary_resampled,
            ...
            train_loader_ori = torch.utils.data.DataLoader(train_dataset_ori, batch_size=batch_size,
                                                   shuffle=True, num_workers=n_workers, pin_memory=True)
            train_dataloader.append(train_loader_ori)
            train_pairs += len(Xtrain_resampled)
    ...
    if val_annotation_filename is not None:
        ...
        if val_annotation_df is not None:
            ...
            val_annotation_df = val_annotation_df_new[train_annotation_df.columns].reset_index(drop=True)
    ...
    if val_annotation_df is not None:
        ...
        val_dataset_ori = ImageLoader2(img_folder, Xval, yval, ybinary_val, material_val,
                                    compression_p = args.compression_p, 
                                    p_range = args.p_range,
                                    transform=transforms.Compose([
                                                ...
                                                transforms.Normalize(mean=transform_mean,
                                            std=transform_std)]))
        val_loader_ori = torch.utils.data.DataLoader(val_dataset_ori, batch_size=(batch_size * 2),
    ...
    val_dataloader.append(val_loader_ori)
    
    if run_mode.startswith('train'):
        if aug_train_pair_data is not None:
            for index, aug_pair_files in enumerate(zip(aug_train_pair_data, aug_val_pair_data)):
                print(f"*********************************************************")
                print(f" this is the {index}th train_test_pair")
                print(f"=========================================================")
                print(f"aug_train_files_{index}:\n {aug_pair_files[0]}\n")
                print(f"aug_val_files_{index}:\n {aug_pair_files[1]}")
                print(f"=========================================================")

                Xtrain_resampled, ytrain_resampled, ybinary_resampled, material_train_resampled, _ = process_csv_files(aug_pair_files[0], 
                                                                                                                args, 
                                                                                                                label_confidence, 
                ...
                aug_train_dataset = ImageLoader2(img_folder, Xtrain_resampled, ytrain_resampled, ybinary_resampled,
                                    material_train_resampled,
                                    compression_p = args.compression_p, 
                                    p_range = args.p_range,
                                    transform=transform_separate,
                ...
                if aug_pair_files[1] is not None:
                    Xval_resampled, yval_resampled, yvalbinary_resampled, material_val_resampled, _ = process_csv_files(aug_pair_files[1], 
                                                                                                                    args, 
                                                                                                                    label_confidence, 
                                                                                                                    defect_class_considered, 
                                                                                                                    defect_code_link, 
                                                                                                                    'val')
                    aug_val_dataset = ImageLoader2(img_folder, Xval_resampled, yval_resampled, yvalbinary_resampled, material_val_resampled,
                                                compression_p = args.compression_p, 
                                                p_range = args.p_range,
                                                transform=transforms.Compose([
                                                            LUT(lut_path=os.path.join(img_folder, 'merged_annotation', 'insp.lut.png'),
                                                            ...
                                                            transforms.Normalize(mean=transform_mean,
                                                        std=transform_std)]))
                    aug_val_loader = torch.utils.data.DataLoader(aug_val_dataset, batch_size=(batch_size * 2),
                                                shuffle=False, num_workers=n_workers, pin_memory=True)
                    val_pairs += len(Xval_resampled)
                    
                else:
                    aug_val_loader = None

                train_dataloader.append(aug_train_loader)
                val_dataloader.append(aug_val_loader)
                
        print(f'||*** n_train_pairs = {train_pairs} ***||')
    ...
    if test_annotation_filenames is not None:
        Xtest, ytest, ybinary_test, material_test, test_pair_data_df = process_csv_files(test_annotation_filenames, 
                                                                                        args, 
                                                                                        label_confidence, 
                                                                                        defect_class_considered, 
                                                                                        defect_code_link, 
    ...
    if test_pair_data_df is not None:
        Xval, yval, yvalraw, ybinary_val, material_val = [], [], [], [], []
        # aug val_data with test ng pairs 考虑把test_pair_data_df中的ng数据单独做一份测试集
    ...
    print(f'n_train={train_pairs}, n_val={val_pairs}, n_test={test_pairs}, n_total={train_pairs + val_pairs + test_pairs}')
    ...
    if run_mode.startswith('train'):
        ...
        if args.resume is None:
            ...
            if "CL" in output_type:
                ...
                val_function = val_no_cl

            for idx, cur_val_datalodaer_ in enumerate(val_dataloader):
                val_losses, val_acc_bi, val_acc_mc, val_recall_bi, val_recall_mclass, val_label_positions, val_imgs = val_function(
                    cur_val_datalodaer_, model, criterion_bos, criterion_bom, gpu_exists, visualize=True, use_amp=use_amp, selection_score=selection_score)
                print(f"|| the {idx}th val_dataset({len(cur_val_datalodaer_.dataset)}): \n"
                    f'Before finetune: val_losses={val_losses}, mclass_val_acc={val_acc_mc}, binary_val_acc={val_acc_bi},  '
                    f'binary recall={val_recall_bi:.3f}%, mclass recall={val_recall_mclass:.3f}%')

            for idx, cur_test_datalodaer_ in enumerate(test_dataloader):
                test_loss, test_acc_bi, test_acc_mc, test_recall_bi, test_recall_mclass, test_label_positions = val_function(
                cur_test_datalodaer_,
                model,
                criterion_bos,
                criterion_bom,
                gpu_exists, use_amp=use_amp, selection_score=selection_score)

                print(f"|| the {idx}th test_dataset({len(cur_test_datalodaer_.dataset)}): \n"
                    f'binary test acc={test_acc_bi:.3f}%, mclass test acc={test_acc_mc:.3f}%,'
                    f'binary test recall={test_recall_bi:.3f}%, mclass test recall={test_recall_mclass:.3f}%,')
        ...
        for epoch in range(start_epoch, epochs):
            ...
            train_iter(train_dataloader, model, criterion_bos, criterion_bom, cl_loss, optimizer, gpu_exists,
