...
import time
import os
from dataloader.image_resampler_pins import mp_weighted_resampler, ImageFusionLoader, stratified_train_val_split, \
...
def main(args):
    ...
    # data augmentation
    transform_mean_gray = [0.456]
    transform_std_gray = [0.224]
    ...
    transform_sync_gray = transforms.Compose([
                                        DiscreteRotate(angles=[0, 180]),
                                        transforms.RandomHorizontalFlip(p=0.5),
                                        transforms.RandomVerticalFlip(p=0.5),
                                        transforms.ColorJitter(
                                        ...
                                        transforms.Normalize(mean=transform_mean_gray,
                                                             std=transform_std_gray)
                                    ])
    ...
    if region == 'singlepinpad':
        ...
        if args.sharp_aug:
            ...
            transform_sync_gray =  transforms.Compose([  # 同步的
                    # transforms.RandomAdjustSharpness(sharpness_factor=args.sharpness_factor, p=args.sharp_p),
                    RandomSharpness(min_factor=args.sharpness_factor_l, max_factor=args.sharpness_factor_u, p=args.sharp_p),
                    transforms.RandomHorizontalFlip(p=0.5),
                    transforms.RandomVerticalFlip(p=0.5),
    ...
    if args.if_save_test_best_model:
        ...
        print(f"len(ref_image_batches) = {len(ref_image_batches)}")
        if len(ref_image_batches) == 0:
            args.if_save_test_best_model = False
    print(f"args.if_save_test_best_model = {args.if_save_test_best_model}")
    ...
    print(f'aug_train_pair_data_filenames data csvs ={aug_train_pair_data_filenames}')
    print(f'aug_val_pair_data_filenames data csvs ={aug_val_pair_data_filenames}')
    print(f'test_annotation_filenames data csvs ={test_annotation_filenames}')
    ...
    if run_mode.startswith('train'):
        ...
        print(f"ori train_dataset nums = {len(Xtrain_resampled)}")
        train_dataset = ImageFusionLoader(img_folder, Xtrain_resampled, ytrain_resampled, ybinary_resampled,
                                    ...
                                    transform_gray = transform_separate,
                                    transform_same=transform_same,
                                    transform_sync=transform_sync,
                                    transform_sync_gray = transform_sync_gray,
    ...
    if test_annotation_filenames is not None:
        ...
        test_dataset = ImageFusionLoader(img_folder, Xtest, ytest, ybinary_test, material_test,
                                    compression_p = cp_ratio, 
                                    p_range = args.p_range,
                                    select_p = args.select_p,
                                    transform=test_transform,
                                    transform_gray = test_gray_transform,
                                    fusion_type = args.fusion_type,
                                    )
    ...
    val_gray_transform=transforms.Compose([
                                  LUT(lut_path=os.path.join(img_folder, 'merged_annotation', 'insp.lut.png'),
                                      lut_p=lut_p),
                                  transforms.Resize((rs_img_size_h, rs_img_size_w)),
                                  transforms.Normalize(mean=transform_mean_gray,
    ...
    val_dataset = ImageFusionLoader( img_folder, Xval, yval, ybinary_val, material_val,
                                compression_p = cp_ratio,   # 指标还是以png为主
                                p_range = args.p_range,
                                select_p = args.select_p,
                                transform=val_transform,
                                transform_gray = val_gray_transform,
                                fusion_type = args.fusion_type,
                              )
    ...
    if 'resnetsp' in backbone_arch and 'CL' not in output_type:
        ...
        model = MPB3net(backbone=backbone_arch, pretrained=pretrained, n_class=n_class, n_units=n_units, output_form=output_type)
    elif 'fusion' in backbone_arch:
        from models.MPB3_fusion import MPB3net
        if args.fusion_type == 'cat':
             img_c = 6
        elif args.fusion_type == 'cat_gray':
             img_c = 4

        model = MPB3net(img_c, backbone=backbone_arch, pretrained=pretrained, n_class=n_class, n_units=n_units, output_form=output_type)
    ...
    if run_mode.startswith('train'):
        ...
        dummy_input1 = torch.randn(1, img_c, rs_img_size_h, rs_img_size_w).cuda()
        dummy_input2 = torch.randn(1, img_c, rs_img_size_h, rs_img_size_w).cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fusion_type', default='match', type=str)
