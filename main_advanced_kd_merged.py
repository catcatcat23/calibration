...
import time
import os
from dataloader.image_resampler import DiscreteRotate, ImageLoader, LUT
from torchvision import transforms
from utils.metrics import FocalLoss, accuracy, classk_metric
from utils.utilities import AverageMeter
import numpy as np
...
def train(train_loader, model, criterion_bos, criterion_bom, optimizer, gpu_exists, use_amp=False, scaler=None,
          kd_config=None):
    losses = AverageMeter()
...
def val(val_loader, model, criterion_bos, criterion_bom, gpu_exists, visualize=False, use_amp=False,
...
def main(args):
    ...
    if region == 'component':
        ...
        LUT(lut_path=os.path.join(img_folder, 'merged_annotation', 'insp.lut.png'), lut_p=lut_p),
            transforms.Resize((int(rs_img_size_h * rs_factor), int(rs_img_size_w * rs_factor))),
            transforms.RandomCrop((rs_img_size_h, rs_img_size_w)),
            DiscreteRotate(angles=[0, 0, 0, 90, 180, 270]),
            transforms.RandomHorizontalFlip(p=0.5),
            transforms.RandomVerticalFlip(p=0.5),
            transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.15, hue=0.1)
        ])
        transform_same = transforms.Compose([
        LUT(lut_path=os.path.join(img_folder, 'merged_annotation', 'insp.lut.png'), lut_p=lut_p),
            transforms.Resize((int(rs_img_size_h + rs_same_offset), int(rs_img_size_w + rs_same_offset))),
            transforms.RandomCrop((rs_img_size_h, rs_img_size_w)),
            transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.15, hue=0.1)
        ])
        ...
        test_annotation_filenames = [
            os.path.join(annotation_folder, f'bbtest_labels_pairs_{region}_nonpaired_resorted.csv')]
        aug_train_pair_data_filenames = [
            os.path.join(img_folder, 'merged_annotation', date,
                         f'aug_train_pair_labels_{region}_nonpaired_resorted.csv'),
            os.path.join(img_folder, 'merged_annotation', date,
        ...
        aug_val_pair_data_filenames = [
            os.path.join(img_folder, 'merged_annotation', date, f'aug_val_pair_labels_{region}_nonpaired_resorted.csv'),
            os.path.join(img_folder, 'merged_annotation', date, f'aug_val_pair_labels_{region}_mergedresorted.csv'),
            os.path.join(img_folder, 'merged_annotation', date,
                         f'aug_val_pair_labels_{region}_240329c_finalresortedv2.csv'),
            os.path.join(img_folder, 'merged_annotation', date,
                         f'aug_test_pair_labels_{region}_240329c_finalresorted.csv'),
            os.path.join(img_folder, 'merged_annotation', date, f'aug_val_pair_labels_{region}_240410debug_final.csv'),
            os.path.join(img_folder, 'merged_annotation', date,
                         f'aug_testpair_labels_{region}_240426syncfly_final.csv'),
        ...
        aug_train_pair_data_filenames = [
            ...
            os.path.join(img_folder, 'merged_annotation', date,
                         f'aug_train_pair_labels_{region}_240819_final_white.csv')
        ]
        ...
        aug_val_pair_data_filenames = [
            ...
            os.path.join(img_folder, 'merged_annotation', date, f'aug_val_pair_labels_{region}_mergedresorted.csv'),
            os.path.join(img_folder, 'merged_annotation', date,
                         f'aug_val_pair_labels_{region}_240328b_finalresorted.csv'),
            os.path.join(img_folder, 'merged_annotation', date,
                         f'aug_test_pair_labels_{region}_240328b_finalresorted.csv'),
            os.path.join(img_folder, 'merged_annotation', date,
                         f'aug_val_pair_labels_{region}_merged_withcpmlresorted.csv'),
            os.path.join(img_folder, 'merged_annotation', date,
                         f'aug_test_pair_labels_{region}_240819_final_white.csv')
        ]
        ...
        aug_train_pair_data_filenames = [
            os.path.join(annotation_folder,
                         f'aug_train_pair_labels_{region}_nonpaired_rgb.csv'),
            os.path.join(annotation_folder,
            ...
            os.path.join(annotation_folder,
                         f'aug_train_pair_labels_{region}_240715_final_RGB.csv'),
            os.path.join(annotation_folder,
                         f'aug_train_pair_labels_{region}_240702_final_RGB.csv'),
            os.path.join(annotation_folder,
                         f'aug_train_pair_labels_{region}_240708_final_RGB.csv'),
            os.path.join(annotation_folder,
                         f'aug_train_pair_labels_{region}_240725_final_rgb.csv'),
            os.path.join(annotation_folder,
                         f'aug_train_pair_labels_singlepinpad_240930_final_rgbv2.csv')
        ]

        if 'DownSample' in version_name:
            aug_train_pair_data_filenames += [
                os.path.join(annotation_folder,
        ...
        aug_val_pair_data_filenames = [
            os.path.join(annotation_folder,
                         f'aug_val_pair_labels_{region}_nonpairedsplit_rgb.csv'),
            os.path.join(annotation_folder,
        ...
        test_annotation_filenames = [
            os.path.join(annotation_folder, f'bbtest_labels_pairs_{region}_230306v2_rgb_final3.csv')]
        if 'slim' in version_name:
        ...
        aug_val_pair_data_filenames = [
            os.path.join(annotation_folder, f'aug_val_pair_labels_{region}_nonpairedsplit_rgb.csv'),
            os.path.join(annotation_folder, f'aug_val_pair_labels_{region}_merged.csv'),
    ...
    defect_class_considered_val = list(defect_code.values())
    ...
    if run_mode.startswith('train'):
        ...
        if aug_train_pair_data_filenames is not None:
            aug_train_pair_data_df_raw = pd.concat(
                [pd.read_csv(f, index_col=0) for f in aug_train_pair_data_filenames]).reset_index(drop=True)
            if label_confidence == 'certain':
                ...
                print(
                    f'aiug train pair certain = {len(aug_train_pair_data_df_certain)} out of {len(aug_train_pair_data_df_raw)}')
            ...
            aug_train_pair_data_df = aug_train_pair_data_df_raw[[y in defect_class_considered for
                                                                 y in aug_train_pair_data_df_raw[
                                                                     'insp_y_raw']]].copy().reset_index(drop=True)
        ...
        train_pair_data = [
            x + ytrain_resampled[i] + ytrain_resampled[i] + [ybinary_resampled[i], material_train_resampled[i]]
            for i, x in enumerate(Xtrain_resampled)]
        train_pair_data_df = pd.DataFrame(train_pair_data,
                                          columns=['ref_image', 'insp_image', 'ref_y', 'insp_y', 'ref_y_raw',
                                                   'insp_y_raw',
    ...
    print(
        f'test: n_defect_pairs={np.sum(ybinary_test)}, n_ok_pairs={len(ybinary_test) - np.sum(ybinary_test)}, n_total_pairs={len(ybinary_test)}')
    # generate pair inputs for test data
    ...
    print(
        f'n_train={len(ybinary_resampled)}, n_val={len(ybinary_val)}, n_test={len(ybinary_test)}, n_total={len(ybinary_val) + len(ybinary_test) + len(ybinary_resampled)}')
    ...
    if 'cbam' in backbone_arch:
        ...
        model = MPB3net(backbone=backbone_arch, pretrained=pretrained, n_class=n_class, n_units=n_units,
    ...
    if kd_ratio > 0:
        ...
        if 'resnetsp' in teacher_backbone_arch:
            from models.MPB3 import MPB3net
            teacher_model = MPB3net(backbone=teacher_backbone_arch, pretrained=False, n_class=n_class,
                                    n_units=teacher_n_units,
                                    output_form=output_type)
        else:
            from models.MPB3 import MPB3net
            teacher_model = MPB3net(backbone=teacher_backbone_arch, pretrained=False, n_class=n_class,
    ...
    if run_mode.startswith('train'):
        ...
        for epoch in range(start_epoch, epochs):
            ...
            test_loss, test_acc_bi, test_acc_mc, test_recall_bi, test_recall_mclass, test_label_positions = val(
                test_loader, model, criterion_bos, criterion_bom, gpu_exists, use_amp=use_amp,
                selection_score=selection_score)
